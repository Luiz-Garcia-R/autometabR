...
metabol.dems <- function(normalized_data, metadata, group_col = "Group",
  ...
  # --- Extract log2 normalized data ---
  if("log2_normalized" %in% names(normalized_data)) {
    data <- normalized_data$log2_normalized
  } else {
    stop("normalized_data must contain 'log2_normalized'")
  }
  ...
  if(!results) {
    ...
    msg <- sprintf("Differentially expressed metabolites between groups %s vs %s: %d significant",
                   groups[1], groups[2], sig_count)
    message(msg)
    invisible(NULL)
  } else {
    return(results_tbl)
